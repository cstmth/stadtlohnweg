...
new #[Layout('layouts::site')] #[Title('security_settings')] class extends Component {
    ...
    public function updatePassword(): void
    {
        ...
        Flux::toast(variant: 'success', text: __('password_updated'));
    }
...
<section class="w-full">
    ...
    >
        ...
        @if ($canManageTwoFactor)
            <section class="mt-12">
                <flux:heading>{{ __('twofactor') }}</flux:heading>
                <flux:subheading>{{ __('twofactor_manage') }}</flux:subheading>

                <div class="flex flex-col w-full mx-auto space-y-6 text-sm" wire:cloak>
                    @if ($twoFactorEnabled)
                        <div class="space-y-4">
                            <flux:text>
                                {{ __('twofactor_pin_hint') }}
                            </flux:text>
        ...
        @if ($canManagePasskeys)
            <section class="mt-12">
                ...
                <flux:subheading>{{ __('passkey_manage_sub') }}</flux:subheading>
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-zinc-500 dark:text-zinc-400 text-xs">
                                            {{ __('added_time', ['time' => $passkey['created_at_diff']]) }}
                                            @if ($passkey['last_used_at_diff'])
                                                <span class="opacity-50 mx-1">/</span>
                                                {{ __('last_used_time', ['time' => $passkey['last_used_at_diff']]) }}
                                            @endif
                                        </p>
                                    </div>
                                </div>
    ...
    >
        <div class="space-y-6">
            <div class="space-y-2">
                <flux:heading size="lg">{{ __('passkey_remove') }}</flux:heading>
                <flux:text>
                    {{ __('passkey_remove_q', ['name' => $deletingPasskeyName]) }}
                </flux:text>
            </div>
